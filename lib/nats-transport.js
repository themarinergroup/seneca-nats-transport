...
module.exports = function(options) {
  ...
  options = seneca.util.deepextend({
    nats: {
      ...
      actionTopicQueue:     null,
      responseTopicQueue:   null
    }
  ...
  seneca.add({role: 'transport', type: 'nats', hook: 'listen'}, function(msg, done) {
    ...
    transpUtils.listen_topics(seneca, msg, clientOpts, function(topic) {
      ...
      nc.subscribe(topicAct, clientOpts.actionTopicQueue ? { 'queue': clientOpts.actionTopicQueue } : null, function(msg) {
  ...
  seneca.add({role: 'transport', type: 'nats', hook: 'client'}, function(msg, done) {
    ...
    function send(spec, topic, sendDone) {
      ...
      nc.subscribe(topicRes, clientOpts.responseTopicQueue ? { 'queue': clientOpts.responseTopicQueue } : null, function(msg) {
